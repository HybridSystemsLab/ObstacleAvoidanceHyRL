classdef ObstacleEnv_SplitEnv < rl.env.MATLABEnvironment
    ...
    methods              
        ...
        function this = ObstacleEnv_SplitEnv()
        ...
        function [Observation,Reward,IsDone,LoggedSignals] = step(this,Action)
            ...
            X = this.State(1);
            Y = this.State(2);
            X0 = [0.2000,    0.5980,   -0.3000,    0.2960]; % Extended set
            X1 = [0.2020,    0.6000,   -0.4960,    0.1000];
            if d_ob == 0 ||  abs(Y) >= 1.5 || X >= 3 || (Y<-.10 && X>X0(2)) || (Y<-0.1 && X<X0(1)) || Y<X0(3) % X0
%             if d_ob == 0 ||  abs(Y) >= 1.5 || X >= 3 || (Y>0.1 && X>X1(2)) || (Y>0.1 && X<X1(1)) || Y>X1(4) % X1
                IsDone = true;
            else
                IsDone = false;
            end
...
%             ystart = -1+rand(); %X1
    ...
    methods               
        ...
        % (optional) Visualization method, uncomment this to visualize
        function plot(this)
...
%             envUpdatedCallback(this)
    ...
    methods (Access = protected)
        ...
        % Set the visualization figure as the current figure, uncomment
        % this to visualize
%         figure(this.h);
%         clf
% 
